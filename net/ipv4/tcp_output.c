		        ...
		        gfp_t gfp_mask)
{
    ...
    if(tcp_sk(sk)->mptcp)
    {
    	path2=tcp_sk(sk)->mptcp->path_index;
    	//printk(KERN_EMERG "we find the tcp_sk(sk)->mptcp->path_index=%u\n",tcp_sk(sk)->mptcp->path_index);
    }
    //else
       	//printk(KERN_EMERG "tcp_sk(sk)->mptcp is null\n");
	if(skb&&(sysctl_mptcp_rmt||sysctl_mptcp_coding)){
	 //    char srcIP[16],dstIP[16];
		...
		if(skb->ifProtectionPacket == 1)
			ifProtectedPacket=1;
		//ly add head flag start
		if(skb->ifProtectionPacket==2)
		{
			ifRMTed=1;
		}
		
    }
    if(skb&&skb->len>0&&sysctl_mptcp_testtcp==1&&skb->counted_skb==270&&path2==2)
	{
		ifcounted=1;
		//printk(KERN_EMERG "skb->counted_skb=%u,path2=2\n",skb->counted_skb);
		//ly add head flag end
	}
    if(skb&&skb->len>0&&sysctl_mptcp_testtcp==2)
	{
		if(sysctl_mptcp_rmt==1)
		{
			if(skb->counted_skb==270&&path2==2)
				ifcounted=1;
		}
		if(sysctl_mptcp_rmt==2)
			if(skb->counted_skb==261&&path2==2)
		        ifcounted=1;
		//printk(KERN_EMERG "skb->counted_skb=%u,path2=2\n",skb->counted_skb);
		//ly add head flag end
	}
	...
	*(((__be16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) |
	...
	if(sysctl_mptcp_rmt||sysctl_mptcp_coding){
		...
		{
			//printk(KERN_EMERG "we can get a retrans packet in tcp_output!!!\n");
			th->res1 = 0xE;
			tp->total_recovery_trans++;
			//ly add 20190427
			tp->total_fuso_recovery_trans++;
			//ly add 20190427
		...
		if(ifRMTed==1)
		{
			//printk(KERN_EMERG "we can get a coding packet in tcp_output!!!\n");
			th->res1=0xC;
			tp->total_recovery_trans++;
			//ly add
			tp->total_coding_recovery_trans++;
			//ly add
			tp->total_recovery_trans_from_last_read++;
		}
 ...
 if(ifcounted==1&&tp->ifdroped!=0)
	 {
	// 	//printk(KERN_EMERG "we can get a skb->counted_skb=30/29,path2=1 in tcp_output!!!\n");
	 	tp->ifdroped=tp->ifdroped-1;
	 	th->res1=0xA;
	}
    ...
    if(sysctl_mptcp_coding)
	{
		...
		if(ifRMTed==1)
		{
		 	...
		 	//printk(KERN_EMERG "++++++++++++tcp_transmit_skb send a packet+++++++++++ startseq=%u,stopseq=%u, seq = %u ,ack_seq = %u, skb->path_mask=0x%08x+++++++++++\n"
		 	//,tcb->seq,tcb->end_seq,ntohl(th->seq),ntohl(th->ack_seq),tcb->path_mask);
		 else
		 {
		 	if(ifProtectedPacket==1)
		 	{
...
unsigned int tcp_current_mss(struct sock *sk)
{
	...
	//ly add
	if(sysctl_mptcp_coding==1||sysctl_mptcp_rmt==1||sysctl_mptcp_testmss||sysctl_mptcp_testtcp==1||sysctl_mptcp_decoding)
	{
		if(mss_now>24)
		{
			//printk(KERN_EMERG "tcp_current_mss_coding=%d!!\n",mss_now-24);
			return mss_now-24;
		}
		else 
			return mss_now;	
	...
	{
		//printk(KERN_EMERG "tcp_current_mss=%d!!\n",mss_now);
		return mss_now;
	}
	//ly add
}
	...
	tcp_for_write_queue_from_safe(skb, next, sk) {
		...
		if (skb->len <= copy) {
			...
			if (!skb_shinfo(skb)->nr_frags) {
				...
				{
					tcp_set_skb_tso_segs(sk, skb, mss_now+24);
				}
				else
					tcp_set_skb_tso_segs(sk, skb, mss_now);
				//ly add
			   ...
			   int push_one, gfp_t gfp)
{
    ...
    //ly add
    unsigned int newmss_now=mss_now+24;
    //ly add
	...
	//if(skb&&skb->ifProtectionPacket==2)
	//{
	//	printk(KERN_EMERG " our coding packet in tcp xmit skb->seq = %u,skb->end_seq = %u,skb->len =%d\n",TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq,skb->len);
	//}
		//ly add
