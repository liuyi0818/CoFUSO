...
void mptcp_ofo_queue(struct sock *meta_sk)
{
	...
	while ((skb = skb_peek(&meta_tp->out_of_order_queue)) != NULL) {
		...
		if(sysctl_mptcp_test)
			{
				if(!tcp_hdr(skb))
				{
					printk(KERN_EMERG "the tcphdr in ofo is null\n");
				}
				else
				{
					if(tcp_hdr(skb)->res1==0xE)
					{
		...
		//ly add
		//printk(KERN_EMERG "in mptcp_ofo_queue,just meta->nxt_rcv,dataseq skb's seq=%u,skb's endseq=%u\n",TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq);
		//printk(KERN_EMERG "in mptcp_ofo_queue,meta->nxt_rcv=%u\n",meta_tp->rcv_nxt);
			//ly add
