					  ...
					  unsigned int *limit)
{
	...
	*limit = 0;
	...
	//ly add
	if(*reinject==1 && (skb->ifProtectionPacket==1||skb->ifProtectionPacket==2))////Retrieved from reinject queue, possibly rmtProtectionPacket
	//ly add
	{
		struct sock *sk;
		int ifFoundPath=0;
	...
	if(RMT_ENABLED&&skb&&skb->ifProtectionPacket==2)
	{
		mss_now=mss_now+24;
	}
