...
/*mengyuan */
//ly add start
int sysctl_mptcp_coding __read_mostly = 0;
int sysctl_mptcp_testmss __read_mostly = 0;
int sysctl_mptcp_testtcp __read_mostly = 0;
int sysctl_mptcp_decoding __read_mostly = 0;
int sysctl_mptcp_test __read_mostly = 0;
//ly add en
...
static struct ctl_table mptcp_table[] = {
	...
	//ly add start
	{ 
        .procname = "mptcp_coding",
		.data = &sysctl_mptcp_coding,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "mptcp_testmss",
		.data = &sysctl_mptcp_testmss,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "mptcp_testtcp",
		.data = &sysctl_mptcp_testtcp,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "mptcp_decoding",
		.data = &sysctl_mptcp_decoding,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
...
void mptcp_destroy_sock(struct sock *sk)
{
	if (is_meta_sk(sk)) {
		...
		//ly add 20190427
	    //__skb_queue_purge(&tcp_sk(sk)->subflowbuffer);
	    //__skb_queue_purge(&tcp_sk(sk)->codingbuffer);
	    //ly add 20190427
...
static int mptcp_alloc_mpcb(struct sock *meta_sk, __u64 remote_key, u32 window)
{
	...
	//ly add
	//skb_queue_head_init(&master_tp->sendforcodingbuffer);
	skb_queue_head_init(&master_tp->subflowbuffer);
	skb_queue_head_init(&master_tp->codingbuffer);
	//printk(KERN_EMERG "mptcp_alloc_mpcb\n");
	//ly add
