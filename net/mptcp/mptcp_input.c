...
static int mptcp_queue_skb(struct sock *sk)
{
	...
	if (!before64(rcv_nxt64, tp->mptcp->map_data_seq + tp->mptcp->map_data_len + tp->mptcp->map_data_fin) ||
		...
		skb_queue_walk_safe(&sk->sk_receive_queue, tmp1, tmp) {
			//ly add
			//printk(KERN_EMERG "in mptcp_queue_skb,before meta->nxt_rcv,tmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//ly add
			__skb_unlink(tmp1, &sk->sk_receive_queue);
			tp->copied_seq = TCP_SKB_CB(tmp1)->end_seq;
			__kfree_skb(tmp1);
	...
	if (before64(rcv_nxt64, tp->mptcp->map_data_seq)) {
		...
		skb_queue_walk_safe(&sk->sk_receive_queue, tmp1, tmp) {
			//ly add
			//printk(KERN_EMERG "in mptcp_queue_skb,out of meta->nxt_rcv,tmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//ly add
			tp->copied_seq = TCP_SKB_CB(tmp1)->end_seq;
			mptcp_prepare_skb(tmp1, sk);
			//ly add
			//printk(KERN_EMERG "in mptcp_queue_skb,out of meta->nxt_rcv,dataseqtmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//ly add
			__skb_unlink(tmp1, &sk->sk_receive_queue);
		...
		skb_queue_walk_safe(&sk->sk_receive_queue, tmp1, tmp) {
            ...
            //ly add
			//printk(KERN_EMERG "in mptcp_queue_skb,just meta->nxt_rcv,tmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//printk(KERN_EMERG "in mptcp_queue_skb,meta->nxt_rcv=%u\n",meta_tp->rcv_nxt);
			//ly add
			tp->copied_seq = TCP_SKB_CB(tmp1)->end_seq;
			mptcp_prepare_skb(tmp1, sk);
			//ly add
			//printk(KERN_EMERG "in mptcp_queue_skb,just meta->nxt_rcv,dataseqtmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//ly add
			__skb_unlink(tmp1, &sk->sk_receive_queue);
			...
			if (!after(TCP_SKB_CB(tmp1)->end_seq, meta_tp->rcv_nxt)) {
				//ly add
				//printk(KERN_EMERG "in mptcp_queue_skbThis segment has already been received,just meta->nxt_rcv,tmp's seq=%u,tmp's endseq=%u\n",TCP_SKB_CB(tmp1)->seq,TCP_SKB_CB(tmp1)->end_seq);
			//printk(KERN_EMERG "in mptcp_queue_skbThis segment has already been received,meta->nxt_rcv=%u\n",meta_tp->rcv_nxt);
			//ly add
				__kfree_skb(tmp1);
				goto next;
			}
			...
			if(sysctl_mptcp_test)
			{
				if(!tcp_hdr(tmp1))
				{
					printk(KERN_EMERG "the tcphdr is null\n");
				}
				else
				{
					if(tcp_hdr(tmp1)->res1==0xE)
					{
						//printk(KERN_EMERG "fuso is succuss in dataque\n");
				        meta_tp->total_fuso_recovery_success_trans++;
				        //meta_tp->ifdroped=6112;
					}
				}
			}
			//ly add
...
//ly add
int coding_mptcp_prevalidate_skb(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct mptcp_cb *mpcb = tp->mpcb;
...
//ly add
int coding_mptcp_set_mapping(struct sk_buff *skb, const struct sock *sk)
{
	const struct tcp_sock *tp = tcp_sk(sk);
	struct tcp_skb_cb *tcb = TCP_SKB_CB(skb);
...
restart:
	...
	skb_queue_walk_safe(&sk->sk_receive_queue, skb, tmp) {
		...
		/* Push a level higher */
		//ly add
	    if(sysctl_mptcp_decoding&&skb&&(skb->ifProtectionPacket==2||skb->ifProtectionPacket==3))
	    {
	    	//printk(KERN_EMERG "enter the coding in data ready\n");
	    	// printk(KERN_EMERG "enter the coding in data ready,TCP_SKB_CB(skb)->seq=%u,end_seq=%u\n",
	    	// 	TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq);
	    	struct tcp_sock *tp = tcp_sk(sk);
			if(skb->ifProtectionPacket==3)
			{
				//printk(KERN_EMERG "enter the decoding in data ready\n");
	    	    //printk(KERN_EMERG "enter the decoding in data ready,TCP_SKB_CB(skb)->seq=%u,end_seq=%u\n",
	    		//TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq);
				struct sk_buff *skb1=skb->decode_skb;
				//struct  sk_buff *skb2=pskb_copy_for_clone(skb, GFP_ATOMIC);
				if(skb1)
				{
					//printk(KERN_EMERG "skb->decode_skb is not null\n");

				    skb1->ifProtectionPacket=1;
				    //printk(KERN_EMERG "decode in data ready.skb1->ifProtectionPacket=%d",skb1->ifProtectionPacket);
				    //printk(KERN_EMERG "^^^^^  clone_skb: subflow level seq = %u , end_seq = %u, len = %u ,truelen=%d^^^^^\n"
				    	//,TCP_SKB_CB(skb1)->seq,TCP_SKB_CB(skb1)->end_seq,TCP_SKB_CB(skb1)->end_seq-TCP_SKB_CB(skb1)->seq,skb1->len);
				    if(coding_mptcp_prevalidate_skb(sk, skb1) != 0)
				    {	
				    	//printk(KERN_EMERG "^^^^^ mptcp_add_meta_ofo_queue skb1: rmt_mptcp_prevalidate_skb failed ^^^^^\n");
				    	__kfree_skb(skb1);
				    }
				    else if(coding_mptcp_set_mapping(skb1,sk) != 0)
				    {	
				    	//printk(KERN_EMERG "^^^^^ mptcp_add_meta_ofo_queue skb1: rmt_mptcp_set_mapping failed ^^^^^\n");
				    	__kfree_skb(skb1);
				    }
			            else if(!(tp->mpcb->in_time_wait) && skb1) /* In time-wait, do not receive data */
				    {
				    	//printk(KERN_EMERG "^^^^^ mptcp_add_meta_ofo_queue clone_skb: data level seq = %u , end_seq = %u, len = %u,truelen=%d ^^^^^\n"
				    	//	,TCP_SKB_CB(skb1)->seq,TCP_SKB_CB(skb1)->end_seq,TCP_SKB_CB(skb1)->end_seq-TCP_SKB_CB(skb1)->seq,skb1->len);

				    	u32 old_rcv_nxt=tcp_sk(meta_sk)->rcv_nxt;
				    	if(!after(TCP_SKB_CB(skb1)->end_seq, tcp_sk(meta_sk)->rcv_nxt))
				    	{
				    	//	printk(KERN_EMERG "^^^^^our coding skb is later ,the tp->rcv_nxt=%u!\n",tcp_sk(meta_sk)->rcv_nxt);
				    		__kfree_skb(skb1);
				    	}
				    	else if(TCP_SKB_CB(skb1)->seq==tcp_sk(meta_sk)->rcv_nxt)
				    	{
				    		...
				    		//printk(KERN_EMERG "^^^^^we entail the skb into the meta_sk receive_queue!,seq=%u,end_seq=%u,tp->rcv_nxt=%u\n",TCP_SKB_CB(skb1)->seq,TCP_SKB_CB(skb1)->end_seq,tcp_sk(meta_sk)->rcv_nxt);
				    		skb_orphan(skb1);
				    		...
				    		if (TCP_SKB_CB(skb)->tcp_flags & TCPHDR_FIN)
				    			mptcp_fin(meta_sk);
				    		//printk(KERN_EMERG "^^^^^after we entail the skb into the meta_sk receive_queue!,tp->rcv_nxt=%u\n",tcp_sk(meta_sk)->rcv_nxt);
				    	   if (!skb_queue_empty(&tcp_sk(meta_sk)->out_of_order_queue))
				                mptcp_ofo_queue(meta_sk);
				        }
				    	else
				    	{
				    		//printk(KERN_EMERG "^^^^^our coding skb have to stroe in out-of-order!,tp->rcv_nxt=%u\n",tcp_sk(meta_sk)->rcv_nxt);
				    		if (after(TCP_SKB_CB(skb1)->seq, tcp_sk(meta_sk)->rcv_nxt))
				    		{
				    			//printk(KERN_EMERG "^^^^^our coding skb have to stroe in out-of-order!^^^^^seq=%u,end_seq=%u,tp->rcv_nxt=%u\n",TCP_SKB_CB(skb1)->seq,TCP_SKB_CB(skb1)->end_seq,tcp_sk(meta_sk)->rcv_nxt);
				    			if(sysctl_mptcp_test)
				    			{
				    				//if(tcp_sk(meta_sk)->ifdroped!=6112)
				    				//{
				    			...
				    			if (!skb_queue_empty(&tcp_sk(meta_sk)->out_of_order_queue))
				    				mptcp_ofo_queue(meta_sk);
				    			//printk(KERN_EMERG "^^^^^after we entail the skb into the meta_sk in out-of-order!,tp->rcv_nxt=%u\n",tcp_sk(meta_sk)->rcv_nxt);
				    		}
				    		else 
				    		{
				    			//printk(KERN_EMERG "whywhywhy???\n");
				    			__kfree_skb(skb1);
				    		}
				    	}
				            //ly add 20190418
				    ...
				    {	
				    	//printk(KERN_EMERG "^^^^^ mptcp_add_meta_ofo_queue clone_skb: !mpcb->in_time_wait=%d ^^^^^\n",!(tp->mpcb->in_time_wait));
				    }  
...
void mptcp_fin(struct sock *meta_sk)
{
	...
	mptcp_purge_ofo_queue(meta_tp);
	//ly add 20190427
	//__skb_queue_purge(&meta_tp->subflowbuffer);
	//__skb_queue_purge(&meta_tp->codingbuffer);
	//ly add 20190427
